...
export default class URLSearchParams {
    ...
    entries() {
        const dict = [];
        for( let x of this.dict ) dict.push( [ ...x ] );
        return dict;
    }

    keys() {
        const keys = [];
        for( let x of this.dict ) keys.push( x[ 0 ] );
        return keys;
    }

    values() {
        const values = [];
        for( let x of this.dict ) values.push( x[ 1 ] );
        return values;
    }

    toString() {
        const pairs = [];
        for( const x of this.dict ) {
            pairs.push( encodeURIComponent( x[ 0 ] ) + '=' + encodeURIComponent( x[ 1 ] ) );
        }
        return pairs.join( '&' );
    }
}
